---
date: 2026-04-27
title: Natural Selection
slug: natural-selection
status: canonical
canonical: true
canonicalLockDate: 2026-04-12
revised: 2026-04-15
cluster: conceptual-foundations
definition: >
  Natural selection is the differential persistence and reproduction of viability-oriented organisation across generations. In APS, selection is the filtering of differences in the capacity of systems to sustain organised persistence. It does not act on isolated traits, genes, or entities in abstraction, but on whole systems capable of sustaining viability-oriented organisation. It therefore presupposes the existence of constraint-closed, agential organisation within which variation, inheritance, and differential outcomes can occur.
inBrief: >
  Natural selection is the differential filtering of viable organisation.
seeAlso:
  - evolution
  - inheritance
  - variation
  - transformation
  - persistence
  - agency
  - constraint-closure
---

## Conventional framing  
Natural selection is typically understood as a primary evolutionary mechanism acting on heritable variation, often formalised as changes in gene frequencies within populations.

## APS reframing  
APS repositions natural selection as a derivative process that operates only within systems already capable of sustaining viability-oriented organisation. Selection does not generate biological organisation but filters among existing forms of organised persistence. What is selected is not traits or genes in isolation, but the capacity of whole systems to maintain viability under given conditions.

## Key Point  
Natural selection in APS is conditional—it operates only within systems capable of sustaining organised persistence and therefore presupposes biological agency.
